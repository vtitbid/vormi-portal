...
class DeleteVolunteerTaskRoute : Route, KoinComponent {

    override val qualifier = named("delete-volunteer-task-route")

    private val volunteerTaskService by inject<VolunteerTaskService>()

    override fun Routing.apply() {
        authenticate {
            delete(Paths.DeleteVolunteerTask) {
                val volunteerTaskId = call.requiredIdParameter

                volunteerTaskService.removeTask(volunteerTaskId).getOrThrow()
