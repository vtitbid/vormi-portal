...
import tech.vtitbid.vormi.server.util.requiredLimitAndOffsetQueryParameters

class GetVolunteerTasksRoute : Route, KoinComponent {

    override val qualifier = named("get-volunteer-tasks-route")

    private val volunteerTaskService by inject<VolunteerTaskService>()

    override fun Routing.apply() {
        get(Paths.GetVolunteerTasks) {
            val (limit, offset) = call.requiredLimitAndOffsetQueryParameters

            val foundTasks = volunteerTaskService.getTasks(limit, offset, requireDto = true).getOrThrow()

            call.respond(foundTasks.requiredDto)
        }
    }
}